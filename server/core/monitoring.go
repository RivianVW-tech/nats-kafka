...
func (server *NATSKafkaBridge) stats() BridgeStats {
	...
	// NATS() uses natsLock, which is safe to take here regardless of
	// whether the caller holds the running lock.
	if nc := server.NATS(); nc != nil {
		ncStats := nc.Stats()
		stats.NATS = &NATSConnStats{
			Connected:    nc.ConnectedUrl() != "",
			ConnectedURL: nc.ConnectedUrl(),
			Reconnects:   ncStats.Reconnects,
