...
func (c *bridgeCollector) Collect(ch chan<- prometheus.Metric) {
	...
	if stats.NATS != nil {
		connected := 0.0
		if stats.NATS.Connected {
			connected = 1.0
		}
		ch <- prometheus.MustNewConstMetric(descNATSConnected, prometheus.GaugeValue, connected)
		ch <- prometheus.MustNewConstMetric(descNATSReconnects, prometheus.CounterValue, float64(stats.NATS.Reconnects))
		ch <- prometheus.MustNewConstMetric(descNATSInMsgs, prometheus.CounterValue, float64(stats.NATS.InMsgs))
		ch <- prometheus.MustNewConstMetric(descNATSOutMsgs, prometheus.CounterValue, float64(stats.NATS.OutMsgs))
		ch <- prometheus.MustNewConstMetric(descNATSInBytes, prometheus.CounterValue, float64(stats.NATS.InBytes))
		ch <- prometheus.MustNewConstMetric(descNATSOutBytes, prometheus.CounterValue, float64(stats.NATS.OutBytes))
	}
}
